...
const deleteUser = async (req, res, next) => {
  ...
  // Bonus: Cascading Delete delete all posts for this user
  const posts = await readFile("model/posts.json");
  const remainingPosts = posts.filter((post) => post.userId.toString() !== userId);
  await writeFile("model/posts.json", remainingPosts);

  res.status(200).json({ message: "User and their posts deleted successfully", user });
};
